...
VK::Buffer VK::MemoryAllocator::CreateBufferUsingStaging(const CommandBuffer& commandBuffer, const Queue& queue, VkDeviceSize bufferSize, VkBufferUsageFlags usageFlags, void* pData) const
{
    ...
    stagingBuffer.CopyToBuffer(commandBuffer, queue, buffer.GetHandle(), bufferSize);
...
VK::Image VK::MemoryAllocator::CreateTexture2D(const std::string& filepath, const CommandPool& commandPool, const Queue& queue) const
{
